...
# 1. Подключение к LM Studio (исправлены параметры)
llm = ChatOpenAI(
    api_key="none",                             # вместо openai_api_key
    base_url="http://192.168.8.11:1234/v1/",   # вместо openai_api_base
    model="qwen/qwen3.5-9b",                   # вместо model_name
    temperature=0.1,
)

# 2. Загрузка документа (исправлен синтаксис)
source = "bau_fahj.pdf"

if source.endswith('.pdf'):      # .endswith() – с маленькой буквы
    loader = PyPDFLoader(source)
elif source.endswith('.docx'):   # правильный отступ (на уровне if)
